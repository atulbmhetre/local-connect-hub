-- Server-side khata amber/red limit ordering (was client-only in VendorSettings).
-- Valid: khata_red_limit = 0 (disabled / both zero) OR
--        (khata_red_limit > khata_amber_limit AND khata_amber_limit >= 0).
...
COMMENT ON FUNCTION public.vendor_update_own(uuid, text, jsonb) IS
  'Vendor self-update. Blocks discoverable/upi_verified/verification_status/subscription fields. Ban + required-photos assert on is_active=true. Enforces khata_red_limit=0 OR (red>amber AND amber>=0). shop_name patch syncs vendor_categories.brand_name.';
