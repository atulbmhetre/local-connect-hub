-- Allow UPI payment claims while order is still accepted (bill sent, not yet Mark Done).
-- Matches cash: customer can pay as soon as a bill exists. Rating remains gated on fulfilled.
...
BEGIN
  ...
  WHERE id = p_request_id
    AND status IN ('accepted', 'fulfilled')
    AND (
...
COMMENT ON FUNCTION public.claim_customer_payment(uuid, text, text, text) IS
  'Customer claims UPI payment on an accepted or fulfilled order. UTR must be exactly 12 digits.';
