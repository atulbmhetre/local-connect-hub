-- Remove dead is_active / profile_status handling from create_customer_request.
-- Design (intentional): discoverable offline vendors remain bookable for
-- Delivery/Appointment (matches Radar). Help excludes offline at discovery,
-- so booking does not need an extra is_active gate.
