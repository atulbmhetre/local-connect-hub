-- Local Feed pagination past the 50-post cap (mirrors IncomingOrdersSection):
--   1) get_local_feed_posts: raise the p_limit ceiling (100 → 200, same
--      ceiling used by get_vendor_incoming_orders) so "Load more" can bump
--      the limit in +50 steps like Incoming Orders does. This re-applies on
--      top of 20260719180001's banned-author exclusion (kept unchanged below
--      — only the ceiling changes).
--   2) get_local_feed_posts_count: total-matching-posts count (identical
--      filters, incl. banned-author exclusion, no limit) so the client can
--      compute truncatedRemaining, mirroring get_vendor_incoming_orders_count.
