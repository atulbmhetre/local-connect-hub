-- Add structured item capture for auto-bill generation
-- Captures customer's menu selections alongside existing text message

-- Add nullable jsonb column to store structured menu item selections
ALTER TABLE public.requests 
ADD COLUMN IF NOT EXISTS items jsonb DEFAULT NULL;

-- Index for querying orders with structured items
CREATE INDEX IF NOT EXISTS requests_items_idx ON public.requests 
USING GIN (items) WHERE items IS NOT NULL;

COMMENT ON COLUMN public.requests.items IS 
  'Structured menu item selections from ParchiSheet: [{"item_id": "uuid", "name": "text", "quantity": int, "unit_price": numeric}]. NULL for free-text-only orders.';

-- Update create_customer_request to accept optional structured items
-- Get the latest version of the function first
...
BEGIN
  ...
  -- Category resolution logic (existing logic from latest migration)
  SELECT category_id, service_mode
  INTO v_category_id, v_service_mode
  FROM public._resolve_booking_category(
    p_vendor_id := p_vendor_id,
    p_hint_category_id := p_category_id,
    p_hint_service_mode := p_service_mode,
    p_delivery_slot := p_delivery_slot,
    p_appointment_time := p_appointment_time
  );

  -- Validate service mode availability (existing logic)
  SELECT vc.available_modes INTO v_category_modes
  FROM public.vendor_categories vc
  WHERE vc.vendor_id = p_vendor_id AND vc.category_id = v_category_id
  LIMIT 1;

  IF v_category_modes IS NOT NULL AND NOT (v_service_mode = ANY(v_category_modes)) THEN
    RAISE EXCEPTION 'service_mode_unavailable';
  END IF;

  -- Insert request with structured items
  INSERT INTO public.requests (
    ...
    customer_latitude, customer_longitude, appointment_instant, category_id, service_mode,
  ...
  VALUES (
    ...
    p_customer_latitude, p_customer_longitude, p_appointment_instant, v_category_id, v_service_mode,
...
COMMENT ON FUNCTION public.create_customer_request(
  ...
  'Create customer request with optional structured menu items for auto-bill generation';
