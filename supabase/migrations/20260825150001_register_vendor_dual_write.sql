-- Phase 2: dual-write UPI / QR / base_type / GPS onto the new vendor_categories
-- row at register_vendor. Every existing parameter and every vendors.* write is
-- unchanged. Readers stay on vendors until Phase 4.
...
BEGIN
  ...
  FOR i IN 1..v_cat_count LOOP
    ...
    INSERT INTO public.vendor_categories (
      ...
      upi_id, upi_qr_url, upi_qr_payee_id, base_type, latitude, longitude
    )
    VALUES (
...
COMMENT ON FUNCTION public.register_vendor(
  ...
  'Atomic vendor registration. Dual-writes UPI/QR/base_type/GPS onto vendor_categories; vendors remains the read source until Phase 4.';
