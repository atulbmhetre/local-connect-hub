-- Store decoded UPI payee ID from vendor QR upload (optional, set at registration).

ALTER TABLE public.vendors
  ADD COLUMN IF NOT EXISTS upi_qr_payee_id text;

-- Replace 15-arg register_vendor with 16-arg version (new param at end).
...
DROP FUNCTION IF EXISTS public.register_vendor(
  ...
  double precision, double precision, text, text, uuid[], text[], text
);
...
COMMENT ON FUNCTION public.register_vendor(
  ...
  'Atomically creates vendor row, vendor_categories, and 7 verification checks. Optional upi_qr_url and upi_qr_payee_id.';
