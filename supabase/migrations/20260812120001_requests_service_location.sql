-- Help-order reach choice: structured service_location on requests.
-- Values: customer_place (vendor comes to customer) | vendor_place (customer visits vendor).

ALTER TABLE public.requests
  ADD COLUMN IF NOT EXISTS service_location text;

COMMENT ON COLUMN public.requests.service_location IS
  'Help reach choice: customer_place = vendor comes to customer; vendor_place = customer visits vendor. NULL for non-help or legacy rows.';

ALTER TABLE public.requests
  DROP CONSTRAINT IF EXISTS requests_service_location_check;

ALTER TABLE public.requests
  ADD CONSTRAINT requests_service_location_check
  CHECK (
    service_location IS NULL
    OR service_location IN ('customer_place', 'vendor_place')
  );
...
COMMENT ON FUNCTION public.create_customer_request(
  ...
  'Customer order/booking insert. p_service_location: customer_place | vendor_place (Help reach choice).';
