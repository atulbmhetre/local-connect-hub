-- Lock down vendors.upi_verified: not settable via generic vendor_update_own patch.
-- New dedicated RPC vendor_verify_upi mirrors UI checks (format + must match saved upi_id).
-- When upi_id changes via patch, force upi_verified = false server-side.
--
-- Audit note (vendor_verification): NOT inserting a new row here.
-- Registration already seeds check_type='upi_format' as passed and
-- 'upi_pennydrop' as dormant. Flipping upi_pennydrop would falsely claim
-- real bank/PSP KYC (Gold tier). No new check_type invented.
...
BEGIN
  ...
  SET
    ...
    END,
    -- Auto-clear verify when saved UPI ID changes (frontend can omit the reset).
    upi_verified = CASE
      WHEN p_patch ? 'upi_id'
...
-- Dedicated verify path. Format matches frontend isValidUpi:
--   /^[\w.\-]{2,256}@[a-zA-Z]{2,64}$/
CREATE OR REPLACE FUNCTION public.vendor_verify_upi(
  p_vendor_id uuid,
  p_vendor_phone text,
  p_upi_id text
)
...
REVOKE ALL ON FUNCTION public.vendor_verify_upi(uuid, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.vendor_verify_upi(uuid, text, text) TO anon, authenticated;

COMMENT ON FUNCTION public.vendor_verify_upi(uuid, text, text) IS
  'Vendor self-service: set upi_verified=true after format + exact match to saved upi_id. Not a PSP/pennydrop check.';
