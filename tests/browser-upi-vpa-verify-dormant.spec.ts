...
const T = Date.now();
const DEVICE_ID = `device_upi_vpa_${T}`;
...
test("vendor UPI VPA button is coming-soon: no edge invoke, no Decentro call", async ({
  page,
}) => {
  const category = await getActiveCategoryByServiceMode("delivery");
  const phone = `9901${String(T).slice(-6)}`;
  const { data: vendor, error } = await supabaseAdmin
    ...
    .insert({
      ...
      is_banned: false,
      upi_id: `dormant${String(T).slice(-6)}@upi`,
    })
    .select("id, phone")
    .single();
  ...
  let edgeHits = 0;
  await page.route("**/functions/v1/upi-vpa-verify**", async (route) => {
    edgeHits += 1;
    await route.abort();
  });
  ...
  const btn = page.getByTestId("upi-vpa-verify-btn");
  await expect(btn).toBeVisible({ timeout: 15000 });
  await btn.click();

  await expect(page.getByText(/UPI verification coming soon/i)).toBeVisible({
    timeout: 8000,
  });
