/**
 * Paused businesses must be hidden from Radar discovery and new booking,
 * while remaining visible on order-history / tracking RPCs.
 */
...
test("RAD-PAUSE-01 — paused business excluded from radar and new booking; order history still visible", async () => {
  const cat = await getActiveCategoryByLabel("Pharmacy");
  const vendorPhone = `99027${String(T).slice(-5)}`;
  const customerPhone = `88027${String(T).slice(-5)}`;
  const customerDevice = `rad-pause-${T}`;
  ...
  const { data: vendor, error: insErr } = await supabaseAdmin
    ...
    .insert({
      phone: vendorPhone,
      name: "Paused Business Radar",
      shop_name: `!RAD-PAUSE-${T}`,
      category: cat.label,
  ...
  const { data: vcPublic, error: vcErr } = await supabase
    .from("vendor_categories")
    .select("id")
    .eq("vendor_id", vendorId)
    .eq("category_id", cat.id)
  ...
  const { data: vendorPublic, error: rlsErr } = await supabase
    .from("vendors")
    .select("id")
    .eq("id", vendorId)
    .maybeSingle();
  expect(rlsErr).toBeNull();
  expect(vendorPublic).toBeNull();
  ...
  const { data: newOrderId, error: bookErr } = await supabaseAdmin.rpc(
    "create_customer_request",
    {
      ...
      p_message: "RAD-PAUSE new booking should fail",
